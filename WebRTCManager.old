...
class WebRTCManager: NSObject {
    ...
    func initMic() {
        guard let pc = peerConnection, pc.signalingState != .closed else {
            print("⚠️ PeerConnection is nil or closed, skipping initMic")
            return
        }
        if pc.transceivers.contains(where: { $0.mediaType == .audio }) {
        ...
        if let transceiver = pc.addTransceiver(of: .audio, init: transceiverInit) {
            transceiver.sender.track = audioTrack
        }
        
        if isMicMuted {
            print("Restoring muted state after reinit")
            setMicEnabled(false)
        }
        
        if isSpeakerOn {
            print("Restoring speaker state after reinit")
            setAudioOutputToSpeaker(enabled: true)
        }
    ...
    func createOffer(completion: @escaping (Result<RTCSessionDescription, Error>) -> Void) {
        guard let pc = peerConnection, pc.signalingState != .closed else {
            completion(.failure(NSError(domain: "WebRTC", code: -2,
                userInfo: [NSLocalizedDescriptionKey: "PeerConnection is closed"])))
            return
        }
    ...
    func setRemoteDescription(sdp: RTCSessionDescription, completion: ((Error?) -> Void)? = nil) {
        guard let pc = peerConnection, pc.signalingState != .closed else {
            completion?(NSError(domain: "WebRTC", code: -2,
                userInfo: [NSLocalizedDescriptionKey: "PeerConnection is closed"]))
            return
        }
    ...
    func close() {
        audioTrack = nil
        if let pc = peerConnection, pc.signalingState != .closed {
            pc.close()
        }
        peerConnection = nil
    }
